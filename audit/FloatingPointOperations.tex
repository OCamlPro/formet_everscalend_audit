...
\noindent\begin{itemize}
  \item \issueMinor{Naming}{The name of the field {\tt nom} should be {\tt num} for ``numerator''.}
\end{itemize}
...
\noindent\begin{itemize}
  \item \issueMajor{Math error}{Comparing numerators and denominators when testing if fractions are equal is incorrect. $eq(\frac{a}{b}, \frac{a \times 2}{b \times 2})$ will return {\tt false} while it should return {\tt true}. The fractions need to be normalized before testing the equality.}
\end{itemize}
...
\noindent\begin{itemize}
  \item \issueMinor{Math issue in {\tt FPO.simplify}}{Dividing the numerator and denominator by their greatest common divisor might make it unnecessary to do the bitshift and avoid losing precision.}
\end{itemize}
