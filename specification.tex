...
\begin{tabularx}{\linewidth}{|l|X|}
  ...
  Collateral & An amount of vTokens that a borrower has to have in order to borrow some amount of real tokens. \\\hline
  ...
  Account health & The sum of the USD value of a user's supplied tokens divided by the sum of the USD value of all the tokens they borrowed. It is used to determine whether a user is eligible for liquidation or not. When the nominator is greater than the denominator in that fraction, we say that the user's account is healthy, otherwise it is unhealthy. \\\hline
...
\end{itemize}
And there are four roles that can be taken by the users and which determine their permissions when it comes to interacting with the system: %refer to IRoles
\begin{itemize}
...
$BC_u$ represents the amount of vTokens that the user can still use as collateral to borrow. Once that amount is reached, the user can no longer perform any action that requires them to have free vTokens like borrowing or withdrawing.
...
\begin{itemize}
  ...
  $$
  The utilisation ratio unifies supply and borrowing demand into a single variable.
...
Where:
\begin{itemize}
  \item $SI_u$: The supply information of the user $u$, which we represent as a collection of couples $(m_{T_i}, vTsa_i)$ with:
  \begin{itemize}
    \item $T_i$: the kind of the supplied token;
    \item $vTsa_i$: the amount of vTokens that the user got from the supply.
  \end{itemize}
  \item $BI_u$: The borrow information of the user $u$, which we represent as a collection of triplets $(m_{T_j}, ba, ind)$ with:
  \begin{itemize}
...
$$
  vTA = R_T \times \ER{T}{vT} \times \LMul{\M{T}}
$$
